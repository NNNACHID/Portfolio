<template>
    <header class="header box">
        <div class="header__left">
            <div class="header__photo"><img
                    src="../assets/photos/nicolas_check.png"
                    alt="profile.jpg" width="150" height="150" class="header__photo-img"></div>
            <div class="header__base-info">
                <h4 class="title titl--h4"> Nicolas <span class="weight--400"> Nachid </span></h4>
                <div class="status"> DevOps & Fullstack developer </div>
                <ul class="header__social">
                    <li><a target="_blank" rel="noopener" href="https://go.adityacprtm.dev/linkedin"
                            title="linkedIn"><em class="font-icon icon-linkedin1"></em></a></li>
                    <li><a target="_blank" rel="noopener" href="https://go.adityacprtm.dev/github" title="Github"><em
                                class="font-icon icon-github1"></em></a></li>
                </ul>
            </div>
        </div>
        <div class="header__right">
            <ul class="header__contact">
                <li><span class="overhead"> Email </span><a href="mailto:aditya@adityacprtm.dev" target="_blank"
                        rel="noopener" title="Email"> aditya@adityacprtm.dev <em class="font-icon icon-mail"></em></a>
                </li>
                <li><span class="overhead"> Location </span> Jakarta, ID <em class="font-icon icon-compass"></em></li>
            </ul>
            <ul class="header__contact">
                <li><span class="overhead"> CV </span><a target="_blank" rel="noopener"
                        href="https://go.adityacprtm.dev/cv" title="CV"> Download <em
                            class="font-icon icon-download"></em></a></li>
                <li><span class="overhead"> Status </span> 🍉 </li>
            </ul>
        </div>
    </header>
</template>